...
/**
 * The ATMS specialization of the RDR class which creates the /All_Data/XXX-RDR_All and /Data_Products/XXX-RDR structures.
 * This overrides the createRawApplicationPackets method, the bulk of the work is still done by the base RDR class.
 * 
 *
 */
public class ATMS_RDR extends RDR {
	 ...
	 */
	public ATMS_RDR(AllData allData, DataProducts dataProds, FixedDomainDescription dev, int scansPerGranule) throws RtStpsException {
		super(null, RDRName.ATMS_Science, allData, dataProds, dev);
		System.out.println("ATMS RDR creation started.");
		this.scansPerGranule = scansPerGranule;
	}
	

	public ATMS_RDR(Stats stats, AllData allData, DataProducts dataProds, FixedDomainDescription dev, int scansPerGranule) throws RtStpsException {
		super(stats, RDRName.ATMS_Science, allData, dataProds, dev);
		if (stats == null) {
			System.out.println("ATMS RDR creation started.");
		}
		this.scansPerGranule = scansPerGranule;
	...
	protected RawApplicationPackets createRawApplicationPackets(RDRName rdrName) {
		...
		RawApplicationPackets rap = new ATMSRawApplicationPackets3(stats, SpacecraftId.npp, getSetNum(), scansPerGranule, getPacketPool());
