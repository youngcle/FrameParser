 ...
 *
 */
public class BufferPacketReader implements Iterator<PacketI>, PacketReaderI {
	private File inputFile;
	private InputStream fis;
	private int index = 0;
	private final static int bufSize = 1024*100;
	private byte[] dataBuf = new byte[bufSize];
	 ...
	 * @throws IOException
	 */
	public BufferPacketReader(PacketFactoryI factory, byte[] inputBuffer, int offset, int length) throws RtStpsException {
		
		
		try {
			fis = new ByteArrayInputStream(inputBuffer, offset, length);
