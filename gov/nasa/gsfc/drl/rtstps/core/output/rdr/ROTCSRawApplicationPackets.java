 ...
 *
 */
public class ROTCSRawApplicationPackets extends RawApplicationPackets {
	...
	private EnumSet<PacketName> sciPacketRange = EnumSet.of(PacketName.NTC); //FIXSET1
	...
	private final int scansPerGranule = 5;
	...
	private static long granuleSize=37405000;

	// Maximum number of OMPS TC packets per granule is the following:
	// (5 groups of CCSDS packets) * (30 packets per CCSDS packet group)
	private static long OMPSGRANULE_MAXPACKETS = 150L;
	 ...
	 */
	public ROTCSRawApplicationPackets(SpacecraftId satellite, int setNum, int scansPerGranule, PacketPool packetPool) {
		super(satellite, RDRName.OMPS_TCScience, setNum, packetPool);
	...
	public ROTCSRawApplicationPackets(Stats stats, SpacecraftId satellite, int setNum, int scansPerGranule, PacketPool packetPool) {
		super(satellite, RDRName.OMPS_TCScience, setNum, packetPool);
	 ...
	 */
	public ROTCSRawApplicationPackets(int allRDRId, int setNum) throws RtStpsException {
		super(allRDRId, setNum);
	}
	
	public ROTCSRawApplicationPackets(int readId, int setNum, boolean usedByGranuleOnly) throws RtStpsException {
	...
	public boolean notFull(Packet p) throws RtStpsException 
	{
		...
		if(this.baseTime == 0l)
		{
		    ...
		    ROTCSGranule.setBaseTime();
		    this.baseTime = ROTCSGranule.getBaseTime();
		    System.out.println("BASE TIME SET TO: " + this.baseTime);
		    
		    currentgranule_startboundary = ROTCSGranule.getStartBoundary(ietTimeCurrentPacket);
		    currentgranule_endboundary = ROTCSGranule.getEndBoundary(ietTimeCurrentPacket);
