 ...
 *
 */
public class RONPSRawApplicationPackets extends RawApplicationPackets {
	...
	private EnumSet<PacketName> sciPacketRange = EnumSet.of(PacketName.NP1); //FIXSET1
	...
	private final int scansPerGranule = 1;
	...
	// Maximum number of OMPS NP packets per granule is the following:
	// (1 group of CCSDS packets) * (2 packets per CCSDS packet group)
	private static long OMPSGRANULE_MAXPACKETS = 2L;
	 ...
	 */
	public RONPSRawApplicationPackets(SpacecraftId satellite, int setNum, int scansPerGranule, PacketPool packetPool) {
		super(satellite, RDRName.OMPS_NPScience, setNum, packetPool);
	...
	public RONPSRawApplicationPackets(Stats stats, SpacecraftId satellite, int setNum, int scansPerGranule, PacketPool packetPool) {
		super(satellite, RDRName.OMPS_NPScience, setNum, packetPool);
	 ...
	 */
	public RONPSRawApplicationPackets(int allRDRId, int setNum) throws RtStpsException {
		super(allRDRId, setNum);
	}
	
	public RONPSRawApplicationPackets(int readId, int setNum, boolean usedByGranuleOnly) throws RtStpsException {
	...
	public boolean notFull(Packet p) throws RtStpsException 
	{
		...
		if(this.baseTime == 0l)
		{
		    ...
		    RONPSGranule.setBaseTime();
		    this.baseTime = RONPSGranule.getBaseTime();
		    System.out.println("BASE TIME SET TO: " + this.baseTime);
		    
		    currentgranule_startboundary = RONPSGranule.getStartBoundary(ietTimeCurrentPacket);
		    currentgranule_endboundary = RONPSGranule.getEndBoundary(ietTimeCurrentPacket);
