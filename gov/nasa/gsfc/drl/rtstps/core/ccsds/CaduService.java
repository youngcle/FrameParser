...
import gov.nasa.gsfc.drl.rtstps.core.FrameBroadcaster;
...
/**
 * This node receives all frames that require CCSDS processing. It sorts
 * frames and redirects them to service processors based on the spacecraft
 * and virtual channel numbers.
 * 
 * 
 */
public final class CaduService extends FrameSenderNode implements FrameReceiver
{
    /**
     * This is a class name for this STPS node type, which is also the
     * element name.
     */
    public static final String CLASSNAME = "cadu_service";
    ...
    public void finishSetup(Configuration configuration) throws RtStpsException
    {
        ...
        for (int n = 0; n < length; n++)
        {
            ...
            {
                svlinks = null;
                throw new RtStpsException("caduService: " + target +
    ...
    public void putFrame(Frame frame) throws RtStpsException
    {
        ...
        /**
         * I created my template CADU using the blank constructor because
         * I am only interested in the CADU header.
         */
        cadu.setFrame(frame);
        ...
        if (cadu.isFillFrame())
        {
            ...
            cadu.setDeleted(true);
            ++fillFrames.value;
        }
        else
        {
            ++outputFrames.value;
            int spid = cadu.getSpacecraft();
            int vcid = cadu.getVirtualChannel();
            lookup.set(spid,vcid);
            FrameReceiver target = outputMap.get(lookup);

            if (target != null)
            {
                target.putFrame(frame);
                if (output != null) output.putFrame(frame);
            }
            else
            {
